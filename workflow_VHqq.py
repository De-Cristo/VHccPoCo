...
def get_Hcc_flag(j1BvsL, btag_cut_T):
    return ak.where(j1BvsL > btag_cut_T, 1, 0)
    
class VHqqBaseProcessor(BaseProcessorABC):
    ...
    def apply_object_preselection(self, variation):
        ...
        myJetTagger = self.params.object_preselection.bJet_algorithm
        self.myJetTagger = self.params.object_preselection.bJet_algorithm
        ...
        if self.newjetdefiniton:
            ...
            # Why not working?
            # print(f"Available keys in JetGood: {self.events.JetGood.fields}")
    ...
    def evaluateBDT(self, data, SRtype):
    ...
    def define_common_variables_before_presel(self, variation):
        ...
        self.events["JetsCvsL"] = CvsLsorted(self.events["JetGood"], self.params.object_preselection.bJet_algorithm)
        self.events["dijet_csort"] = get_dijet(self.events.JetsCvsL, self.params.object_preselection.bJet_algorithm)
        ...
        self.events["Hcc_flag"] = get_Hcc_flag(self.events.dijet_bsort.j1BvsL, self.events.btag_cut_T)
...
#                 variables_for_MVA_eval = ak.zip({v:self.events[v] for v in variables_for_MVA_eval_list})  #TODO: use odd_events instead
        ...
        if self.proc_type=="WLNu":
            ...
            self.events["NaL"] = get_additionalleptons( self.events.ElectronGood, self.events.MuonGood, 1) # number of additional leptons
            ...
            self.events["deltaPhi_jet1_MET"] = np.abs(self.events.MET_used.phi - self.events.dijet.j1Phi)            
            
            # self.events["deltaPhi_jet2_MET"] = np.abs(self.events.MET.delta_phi(self.events.JetGood[1]))
            self.events["deltaPhi_jet2_MET"] = np.abs(self.events.MET_used.phi - self.events.dijet.j2Phi)            
            ...
            self.events["deltaPhi_l1_b1"] = np.abs(delta_phi(self.events.lead_lep.phi, self.events.dijet_bsort.j1Phi))
            ...
            if self.myJetTagger == "PNet":
                self.events["b_CvsL"] = self.events.b_jet["btagPNetB"]
                self.events["b_CvsB"] = self.events.b_jet["btagPNetCvL"]
                self.events["b_Btag"] = self.events.b_jet["btagPNetCvB"]
            elif self.myJetTagger == "DeepFlav":
                self.events["b_CvsL"] = self.events.b_jet["btagDeepFlavB"]
                self.events["b_CvsB"] = self.events.b_jet["btagDeepFlavCvL"]
                self.events["b_Btag"] = self.events.b_jet["btagDeepFlavCvB"]
            elif self.myJetTagger == "RobustParT":
                self.events["b_CvsL"] = self.events.b_jet["btagRobustParTAK4B"]
                self.events["b_CvsB"] = self.events.b_jet["btagRobustParTAK4CvL"]
                self.events["b_Btag"] = self.events.b_jet["btagRobustParTAK4CvB"]
            else:
                raise NotImplementedError(f"This tagger is not implemented: {self.myJetTagger}")   
            ...
            self.events["top_mass"] = self.events.top_candidate.mass
    ...
    #                         if self.run_gnn:
    #                             self.events["GNN_Hbb"] = self.evaluateGNN(ak_gnn)
    #                         else:
    #                             self.events["GNN_Hbb"] = np.zeros_like(self.events["BDT_Hbb"])
        ...
        if self.proc_type=="ZNuNu":
            ...
            self.events["VHbb_pt_ratio"] = self.events.ZHbb_pt_ratio
            self.events["ZHbb_deltaPhi"] = np.abs(self.events.Z_candidate.delta_phi(self.events.dijet_bsort))
            self.events["VHbb_deltaPhi"] = self.events.ZHbb_deltaPhi
            
            self.events["deltaPhi_jet1_MET"] = np.abs(delta_phi(self.events.Z_candidate.phi, self.events.dijet_csort.j1Phi))
            self.events["deltaPhi_jet2_MET"] = np.abs(delta_phi(self.events.Z_candidate.phi, self.events.dijet_csort.j2Phi))
            
            self.events["deltaPhi_b1_MET"] = np.abs(delta_phi(self.events.Z_candidate.phi, self.events.dijet_bsort.j1Phi))
            self.events["deltaPhi_b2_MET"] = np.abs(delta_phi(self.events.Z_candidate.phi, self.events.dijet_bsort.j2Phi))
